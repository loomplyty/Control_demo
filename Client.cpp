...
int OnConnDataReceived(Aris::Core::CONN *pConn, Aris::Core::MSG &data)
{
	...
	{
	//    cout<<"rt msg get in client,msg length is :"<<data.GetLength()<<endl;
	/*    int mode,state,pos,vel,current;

       data.PasteAt(&state,sizeof(int),0);
       data.PasteAt(&mode,sizeof(int),sizeof(int));
       data.PasteAt(&pos,sizeof(int),sizeof(int)*2);
       data.PasteAt(&vel,sizeof(int),sizeof(int)*3);
       data.PasteAt(&current,sizeof(int),sizeof(int)*4);
...
int OnControlCommandNeeded(Aris::Core::MSG &msg)
{
	int cmd;
	cout<<"Commands:"<<endl;
	cout<<"1.PowerOff"<<endl<<"2.Stop"<<endl<<"3.Enable"<<endl<<"4.Running"<<endl<<"5.Gohome_1"<<endl<<"6.Gohome_2"<<endl<<"7.HOme2start_1"<<endl<<"8.Home2start_2"<<endl<<"9.Forward"<<endl<<"10.Backward"<<endl<<"11.Fast_Forward"<<endl<<"12.Fast_Backward"<<endl<<"13.Legup"<<endl<<"14.Turnleft"<<endl<<"15.TurnRight"<<endl<<"16.Online Gait"<<endl<<"17. To standstill"<<endl<<"18. Torque Test"<<endl;
...
//	static int count;
   // count++;
	cout<<"in call back function :rt msg get in client,msg length is :"<<msg.GetLength()<<endl;
