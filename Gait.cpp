...
int GaitTestTraj[GAIT_TESTTRAJ_LEN][GAIT_WIDTH];
 ...
 int GaitForwardAccTraj[GAIT_FORWARD_ACC_LEN][GAIT_WIDTH];
int GaitForwardConTraj[GAIT_FORWARD_CON_LEN][GAIT_WIDTH];
int GaitForwardDecTraj[GAIT_FORWARD_DEC_LEN][GAIT_WIDTH];

 int GaitBackwardAccTraj[GAIT_BACKWARD_ACC_LEN][GAIT_WIDTH];
int GaitBackwardConTraj[GAIT_BACKWARD_CON_LEN][GAIT_WIDTH];
int GaitBackwardDecTraj[GAIT_BACKWARD_DEC_LEN][GAIT_WIDTH];
...
int CGait::InitGait(Aris::RT_CONTROL::CSysInitParameters& param)
{
		...
		fin.open("../../resource/gait/Home2Start.txt");
		...
		fin.open("../../resource/gait/TrajTest.txt");

		for(int i=0;i<GAIT_TESTTRAJ_LEN;i++)
		{
			fin>>Line_Num;
			...
			{
				fin>>temp;
				GaitTestTraj[i][j]=(int)temp;
			   // cout<<"gait test "<<GaitTestTraj[i][j]<<endl;
		...
		fin.open("../../resource/gait/ForwardAccTraj.txt");

		for(int i=0;i<GAIT_FORWARD_ACC_LEN;i++)
		{
			fin>>Line_Num;
			...
			{
				fin>>temp;
				GaitForwardAccTraj[i][j]=(int)temp;
			  //cout<<"gait forward "<<GaitForwardTraj[i][j]<<endl;
		...
		fin.open("../../resource/gait/ForwardConTraj.txt");

		for(int i=0;i<GAIT_FORWARD_CON_LEN;i++)
		{
			fin>>Line_Num;
			...
			{
				fin>>temp;
				GaitForwardConTraj[i][j]=(int)temp;
			  //cout<<"gait forward "<<GaitForwardTraj[i][j]<<endl;
		...
		fin.open("../../resource/gait/ForwardDecTraj.txt");

		for(int i=0;i<GAIT_FORWARD_DEC_LEN;i++)
		{
			fin>>Line_Num;
			...
			{
				fin>>temp;
				GaitForwardDecTraj[i][j]=(int)temp;
			  //cout<<"gait forward "<<GaitForwardTraj[i][j]<<endl;
	...
	/*	fin.open("../../resource/gait/BackwardTraj.txt");

		for(int i=0;i<GAIT_BACKWARD_LEN;i++)
		{
			fin>>Line_Num;
			...
			{
				fin>>temp;
				GaitBackwardTraj[i][j]=(int)temp;
			  // cout<<"gait backward "<<GaitForwardTraj[i][j]<<endl;
...
int CGait::RunGait(EGAIT* p_gait,Aris::RT_CONTROL::CMachineData& p_data)
{
		...
		if(isReadytoSetGait[i]==true)
		{
			...
			case GAIT_HOME:
				    ...
				    if(p_data.isMotorHomed[i]==true)
					{
					    ...
					    if(motorNumHomed==18)
					    {
					    	rt_printf("**********MOTORS ARE ALL HOMED, READY TO SET GAIT**********\n");
			...
			case GAIT_BACK_TO_HOME:

				if(p_gait[i]!=m_currentGait[i])
				{
					rt_printf("driver %d: GAIT_BACK TO HOME begin\n",i);
					m_gaitState[i]=EGaitState::GAIT_RUN;
					m_currentGait[i]=p_gait[i];
					m_gaitStartTime[i]=p_data.time;
				    m_commandDataMapped[motorID].Position=GaitHome2Start[GAIT_HOME2START_LEN-1][motorID];
				...
				else
				{
			    	...
			    	m_commandDataMapped[motorID].Position=GaitHome2Start[GAIT_HOME2START_LEN-1-m_gaitCurrentIndex[i]][motorID];


					if(m_gaitCurrentIndex[i]==GAIT_HOME2START_LEN-1)
					{
						rt_printf("driver %d:GAIT_HOME2START will transfer to GAIT_STANDSTILL, HOME POSITION ACHIEVED...\n",i);
						p_gait[i]=GAIT_STANDSTILL;
			...
			case GAIT_TEST_TRAJ:
				...
				if(p_gait[i]!=m_currentGait[i])
				{
					rt_printf("driver %d: GAIT_TEST_TRAJ begin\n",i);
					m_gaitState[i]=EGaitState::GAIT_RUN;
					m_currentGait[i]=p_gait[i];
					m_gaitStartTime[i]=p_data.time;
				    m_commandDataMapped[motorID].Position=GaitTestTraj[0][motorID];
				...
				else
				{
			    	...
			    	m_commandDataMapped[motorID].Position=GaitTestTraj[m_gaitCurrentIndex[i]][motorID];

					if(m_gaitCurrentIndex[i]==GAIT_TESTTRAJ_LEN-1)
					{
						rt_printf("driver %d:GAIT_TESTTRAJ will transfer to GAIT_STANDSTILL...\n",i);
						p_gait[i]=GAIT_STANDSTILL;
			...
			case GAIT_GO_TXT_TRAJ:

				if(p_gait[i]!=m_currentGait[i])
				{
					rt_printf("driver %d: GAIT_GO_TXT_TRAJ begin\n",i);
					m_gaitState[i]=EGaitState::GAIT_RUN;
					m_currentGait[i]=p_gait[i];
					m_gaitStartTime[i]=p_data.time;
				    m_commandDataMapped[motorID].Position=GaitTxtTraj[0][motorID];
				...
				else
				{
			    	...
			    	m_commandDataMapped[motorID].Position=GaitTxtTraj[m_gaitCurrentIndex[i]][motorID];


					if(m_gaitCurrentIndex[i]==GAIT_TXT_TRAJ_LEN-1)
					{
						rt_printf("driver %d:GAIT_GO_TXT_TRAJ will transfer to GAIT_STANDSTILL...\n",i);
						p_gait[i]=GAIT_STANDSTILL;
			...
			case GAIT_FORWARD_TRAJ:


				if(p_gait[i]!=m_currentGait[i])
				{
					rt_printf("driver %d: GAIT_FORWARD begin\n",i);
					m_gaitState[i]=EGaitState::GAIT_RUN;
					m_currentGait[i]=p_gait[i];
					m_gaitStartTime[i]=p_data.time;
					m_commandDataMapped[motorID].Position=GaitForwardAccTraj[0][motorID];
				...
				else
				{
					...
					if(m_gaitCurrentIndex[i]<GAIT_FORWARD_ACC_LEN)
					{
						m_commandDataMapped[motorID].Position=GaitForwardAccTraj[m_gaitCurrentIndex[i]][motorID];
					}
					else if(m_gaitCurrentIndex[i]<GAIT_FORWARD_ACC_LEN+GAIT_FORWARD_CON_LEN)
					{
						if(m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN==0)
						{
							Gait_iter_count[i]=1;
							if(i==0)
								rt_printf("constant phsase count %d\n",Gait_iter_count[i]);

						}


						m_commandDataMapped[motorID].Position=GaitForwardConTraj[m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN][motorID];

					}

					else if(IsConsFinished[i]==false)
					{
						if(((m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN)%GAIT_FORWARD_CON_LEN)==0)
						{
							Gait_iter_count[i]++;
							if(i==0)
								rt_printf("constant phsase count %d\n",Gait_iter_count[i]);
						...
						m_commandDataMapped[motorID].Position=GaitForwardConTraj[(m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN)%GAIT_FORWARD_CON_LEN][motorID];
					}
					else if(m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN-GAIT_FORWARD_CON_LEN*Gait_iter_count[i]<0)
						m_commandDataMapped[motorID].Position=GaitForwardConTraj[(m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN)%GAIT_FORWARD_CON_LEN][motorID];
					else
					{
				    	m_commandDataMapped[motorID].Position=GaitForwardDecTraj[(m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN-GAIT_FORWARD_CON_LEN*Gait_iter_count[i])][motorID];
				    	if((m_gaitCurrentIndex[i]-GAIT_FORWARD_ACC_LEN-GAIT_FORWARD_CON_LEN*Gait_iter_count[i])==0&&i==0)
				    		rt_printf("Beginning Deceleration phase...\n");

					}



					if(m_gaitCurrentIndex[i]==GAIT_FORWARD_ACC_LEN+GAIT_FORWARD_CON_LEN*Gait_iter_count[i]+GAIT_FORWARD_DEC_LEN-1)
				    {
						rt_printf("driver %d: GAIT_FORWARD will transfer to GAIT_STANDSTILL...\n",i);
						p_gait[i]=GAIT_STANDSTILL;
	...
	/*		case GAIT_BACKWARD_TRAJ:

				if(p_gait[i]!=m_currentGait[i])
				{
					rt_printf("driver %d: GAIT_BACKWARD_TRAJ begin\n",i);
					m_gaitState[i]=EGaitState::GAIT_RUN;
					m_currentGait[i]=p_gait[i];
					m_gaitStartTime[i]=p_data.time;
				    m_commandDataMapped[motorID].Position=GaitBackwardTraj[0][motorID];
				...
				else
				{
			    	...
			    	m_commandDataMapped[motorID].Position=GaitBackwardTraj[m_gaitCurrentIndex[i]][motorID];

					if(m_gaitCurrentIndex[i]==GAIT_BACKWARD_LEN-1)
					{
						rt_printf("driver %d:GAIT_BACKWARDTRAJ will transfer to GAIT_STANDSTILL...\n",i);
						p_gait[i]=GAIT_STANDSTILL;
